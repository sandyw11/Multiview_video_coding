...
#define TZ_SEARCH_CONFIGURATION                                                                                 \
  const Int  iRaster                  = 5;  /* TZ soll von aussen übergeben werden */                           \
...
/*__inline
Void MotionEstimation::xTZHorhexSearch( IntTZSearchStrukt& rcStrukt, SearchRect rcSearchRect,
                                        const Int iStartX, const Int iStartY, const Int iDist )
{ // 8 point search,                   //   1 2 3
  // search around the start point     //   4 0 5
  // with the required  distance       //   6 7 8
  ...
  else // iDist == 1
  {
	...
	if ( iDist <= 8 ) //6PointHexagon
	{
      if (  iTop >= -rcSearchRect.iNegVerLimit && iLeft   >= -rcSearchRect.iNegHorLimit &&
            iRight <=  rcSearchRect.iPosHorLimit && iBottom <= rcSearchRect.iPosVerLimit ) // check border
      {
        xTZSearchHelp( rcStrukt, iLeft_2,  iTop,      1, iDist );
        xTZSearchHelp( rcStrukt, iRight_2, iTop,      3, iDist );
        xTZSearchHelp( rcStrukt, iLeft,    iStartY,   4, iDist );
        xTZSearchHelp( rcStrukt, iRight,   iStartY,   5, iDist );
        xTZSearchHelp( rcStrukt, iLeft_2,  iBottom,   6, iDist );
        xTZSearchHelp( rcStrukt, iRight_2, iBottom,   8, iDist );
      }
      else // check border
      {
	    if ( iTop >= -rcSearchRect.iNegVerLimit ) // check top
	...
	{  
	  const Int iLeft_1    = iStartX - (iDist>>3);
	  const Int iLeft_3    = iStartX - (iDist>>2) * 3;
      const Int iRight_1   = iStartX + (iDist>>3);
      const Int iRight_3   = iStartX + (iDist>>2) * 3;
           ...
           iRight <=  rcSearchRect.iPosHorLimit && iBottom <= rcSearchRect.iPosVerLimit ) // check border
      {
        ...
        xTZSearchHelp( rcStrukt, iLeft_1,  iTop,      0, iDist );
        xTZSearchHelp( rcStrukt, iRight_1, iTop,      0, iDist );
        xTZSearchHelp( rcStrukt, iLeft_3,  iTop_2,    0, iDist );
        xTZSearchHelp( rcStrukt, iRight_3, iTop_2,    0, iDist );
	    xTZSearchHelp( rcStrukt, iLeft_1,  iBottom,   0, iDist );
        xTZSearchHelp( rcStrukt, iRight_1, iBottom,   0, iDist );
        xTZSearchHelp( rcStrukt, iLeft_3,  iBottom_2, 0, iDist );
        xTZSearchHelp( rcStrukt, iRight_3, iBottom_2, 0, iDist );
      }
      else // check border
      {
	  	if ( iTop >= -rcSearchRect.iNegVerLimit ) // check top
        ...
        if ( iLeft_1 >= -rcSearchRect.iNegHorLimit ) // check Left_1
        {
          if ( iTop >= -rcSearchRect.iNegVerLimit ) // check Top
          {
            xTZSearchHelp( rcStrukt, iLeft_1, iTop, 0, iDist );
        ...
        if ( iLeft_3 >= -rcSearchRect.iNegHorLimit ) // check Left_3
        {
          if ( iTop_2 >= -rcSearchRect.iNegVerLimit ) // check Top_2
          {
            xTZSearchHelp( rcStrukt, iLeft_3, iTop_2, 0, iDist );
          }
          if ( iBottom_2 <= rcSearchRect.iPosVerLimit ) // check Bottom_2
          {
            xTZSearchHelp( rcStrukt, iLeft_3, iBottom_2, 0, iDist );
          }
        } 		  
        if ( iRight_1 <= rcSearchRect.iPosHorLimit ) // check Right_1
        {
          if ( iTop >= -rcSearchRect.iNegVerLimit ) // check Top
          {
            xTZSearchHelp( rcStrukt, iRight_1, iTop, 0, iDist );
        ...
        if ( iRight_3 <= rcSearchRect.iPosHorLimit ) // check Right_3
        {
          if ( iTop_2 >= -rcSearchRect.iNegVerLimit ) // check Top_2
          {
            xTZSearchHelp( rcStrukt, iRight_3, iTop_2, 0, iDist );
...
__inline
Void MotionEstimation::xTZ4PointDiamondSearch( IntTZSearchStrukt& rcStrukt, SearchRect rcSearchRect,
                                        const Int iStartX, const Int iStartY, const Int iDist )
{ // 4 point search,                   //     2 
  // search around the start point     //   4 0 5
  // with the required  distance       //     7  
  AOT_DBG( iDist == 0 );
  const Int iTop        = iStartY - iDist;
  const Int iBottom     = iStartY + iDist;
  const Int iLeft       = iStartX - iDist;
...
__inline
Void MotionEstimation::xTZOctSearch( IntTZSearchStrukt& rcStrukt, SearchRect rcSearchRect,
                                        const Int iStartX, const Int iStartY, const Int iDist , const UChar tab)
{ // 8 point search,                   //   1 2 3
  // search around the start point     //   4 0 5
  // with the required  distance       //   6 7 8
  AOT_DBG( iDist == 0 );
  const Int iTop        = iStartY - (iDist<<1);
  const Int iBottom     = iStartY + (iDist<<1);
  const Int iLeft       = iStartX - (iDist<<1);
  const Int iRight      = iStartX + (iDist<<1);
  const Int iTop_2      = iStartY - iDist;
  const Int iBottom_2   = iStartY + iDist;
  const Int iLeft_2     = iStartX - iDist;
  const Int iRight_2    = iStartX + iDist;

  rcStrukt.uiBestRound += 1;

    if ( tab == 2 )
    {
	  if (  iTop >= -rcSearchRect.iNegVerLimit && iRight <= rcSearchRect.iPosHorLimit && iLeft >= -rcSearchRect.iNegHorLimit ) // check border
      {
        xTZSearchHelp( rcStrukt, iRight_2, iTop,   2, (iDist<<1) );  //1
        xTZSearchHelp( rcStrukt, iRight,   iTop_2, 2, (iDist<<1) );  //4
        xTZSearchHelp( rcStrukt, iLeft_2,  iTop,   2, (iDist<<1) );  //3
        xTZSearchHelp( rcStrukt, iLeft,    iTop_2, 2, (iDist<<1) );  //5
      }
	  else
	  {
		if ( iTop >= -rcSearchRect.iNegVerLimit ) 
		{
		  if ( iRight_2 <= rcSearchRect.iPosHorLimit )  
		  {
            xTZSearchHelp( rcStrukt, iRight_2, iTop, 2, (iDist<<1) );
		  }
		  if ( iLeft_2 >= -rcSearchRect.iNegHorLimit ) 
          {
            xTZSearchHelp( rcStrukt, iLeft_2, iTop, 2, (iDist<<1) );
          }
        }
        if ( iTop_2 >= -rcSearchRect.iNegVerLimit ) 
		{
		  if ( iRight <= rcSearchRect.iPosHorLimit )  
		  {
            xTZSearchHelp( rcStrukt, iRight, iTop_2, 2, (iDist<<1) );
		  }
		  if ( iLeft >= -rcSearchRect.iNegHorLimit ) 
          {
            xTZSearchHelp( rcStrukt, iLeft, iTop_2, 2, (iDist<<1) );
          }
        }
	  }
    }
	...
	if ( tab == 7 )
	{
	  if ( iBottom <= rcSearchRect.iPosVerLimit && iRight <= rcSearchRect.iPosHorLimit && iLeft >= -rcSearchRect.iNegHorLimit ) // check border
	  {
	    xTZSearchHelp( rcStrukt, iRight_2, iBottom,   7, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iLeft_2,  iBottom,   7, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iRight,   iBottom_2, 7, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iLeft,    iBottom_2, 7, (iDist<<1) );
	  }
	  else
	  {
	    if ( iBottom <= rcSearchRect.iPosVerLimit )
		{
		  if ( iRight_2 <= rcSearchRect.iPosHorLimit )
		  {
            xTZSearchHelp( rcStrukt, iRight_2, iBottom, 7, (iDist<<1) );
		...
		if ( iBottom_2 <= rcSearchRect.iPosVerLimit )
        {
 		  if ( iRight <= rcSearchRect.iPosHorLimit )
		  {
            xTZSearchHelp( rcStrukt, iRight, iBottom_2, 7, (iDist<<1) );
	...
	if ( tab == 4 )
	{
	  if ( iLeft >= -rcSearchRect.iNegHorLimit && iTop >= -rcSearchRect.iNegVerLimit && iBottom <= rcSearchRect.iPosVerLimit ) // check border
	  {
	    xTZSearchHelp( rcStrukt, iLeft,   iTop_2,    4, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iLeft,   iBottom_2, 4, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iLeft_2, iTop,      4, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iLeft_2, iBottom,   4, (iDist<<1) );
	  }
	  else
	  {
	    if ( iLeft >= -rcSearchRect.iNegHorLimit )
        {
          if ( iTop_2 >= -rcSearchRect.iNegVerLimit )
          {
            xTZSearchHelp( rcStrukt, iLeft, iTop, 4, (iDist<<1) );
          }
          if ( iBottom_2 <= rcSearchRect.iPosVerLimit )
          {
            xTZSearchHelp( rcStrukt, iLeft, iTop, 4, (iDist<<1) );
		...
		if ( iLeft_2 >= -rcSearchRect.iNegHorLimit ) // check left
        {
          if ( iTop >= -rcSearchRect.iNegVerLimit )
          {
            xTZSearchHelp( rcStrukt, iLeft_2, iTop, 4, (iDist<<1) );
	...
	if ( tab == 5 )
	{
	  if ( iRight <= rcSearchRect.iPosHorLimit && iTop >= -rcSearchRect.iNegVerLimit && iBottom <= rcSearchRect.iPosVerLimit ) // check border
	  {
	    xTZSearchHelp( rcStrukt, iRight,   iTop_2,    5, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iRight,   iBottom_2, 5, (iDist<<1) );
	    xTZSearchHelp( rcStrukt, iRight_2, iTop,      5, (iDist<<1) );
		xTZSearchHelp( rcStrukt, iRight_2, iBottom,   5, (iDist<<1) );
	  }
	  else
	  {
	    if ( iRight <= rcSearchRect.iPosHorLimit ) 
...
Void MotionEstimation::xTZSearch( IntYuvPicBuffer *pcPelData, Mv& rcMv, UInt& ruiSAD, UInt uiSearchRange )
{
  ...
  // fist search
  for( iDist = 1; iDist <= iRaster ; iDist*=2 )
  {
    if( bFirstSearchDiamond == 1 )
    {
      xTZ4PointDiamondSearch( cStrukt, cSearchRect, iStartX, iStartY, iDist );
	  ...
	  {
	    xTZOctSearch( cStrukt, cSearchRect, iStartX, iStartY, iDist, cStrukt.ucPointNr );
	  }
	}
    else
    {
